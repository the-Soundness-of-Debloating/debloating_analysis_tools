...
clang::SourceLocation SourceManager::FindLocationAfterCond(const clang::SourceManager &SM, clang::Expr *E) {
    clang::SourceLocation L = E->getEndLoc();
    if (L.isMacroID())
        L = SM.getFileLoc(L);
    L = clang::Lexer::findLocationAfterToken(L, clang::tok::r_paren, SM, clang::LangOptions(), false);
    if (L.isMacroID())
        L = SM.getFileLoc(L);
    return L;
}

clang::SourceLocation SourceManager::GetEndOfCond(const clang::SourceManager &SM, clang::Expr *E) {
...
clang::SourceLocation SourceManager::GetEndLocationUntil(const clang::SourceManager &SM,
...
clang::SourceLocation SourceManager::GetEndLocation(clang::ASTContext *Context, clang::SourceLocation Loc) {
    const clang::SourceManager &SM = Context->getSourceManager();
    clang::Token Tok;

    clang::SourceLocation Beginning = clang::Lexer::GetBeginningOfToken(Loc, SM, Context->getLangOpts());
    clang::Lexer::getRawToken(Beginning, Tok, SM, Context->getLangOpts());

    clang::SourceLocation End;
    if (Tok.getKind() == clang::tok::semi || Tok.getKind() == clang::tok::r_brace) {
        End = Loc;
    } else {
...
clang::SourceLocation SourceManager::GetRealLocation(clang::ASTContext *Context, clang::SourceLocation Loc) {
    const clang::SourceManager &SM = Context->getSourceManager();
    if (Loc.isMacroID())
...
clang::SourceLocation SourceManager::GetBeginOfStmt(clang::ASTContext *Context, clang::Stmt *S) {
    const clang::SourceManager &SM = Context->getSourceManager();
...
clang::SourceLocation SourceManager::GetEndOfStmt(clang::ASTContext *Context, clang::Stmt *S) {
    const clang::SourceManager &SM = Context->getSourceManager();
...
llvm::StringRef SourceManager::GetSourceText(const clang::SourceManager &SM, const clang::SourceLocation &B,
                                             const clang::SourceLocation &E) {
    return clang::Lexer::getSourceText(clang::CharSourceRange::getCharRange(B, E.getLocWithOffset(1)), SM,
                                       clang::LangOptions());
}

llvm::StringRef SourceManager::GetSourceText(const clang::SourceManager &SM, const clang::SourceRange &SR) {
    return GetSourceText(SM, SR.getBegin(), SR.getEnd());
}

std::pair<clang::SourceLocation, clang::SourceLocation>
SourceManager::getStartAndEnd(clang::ASTContext *context,
                              llvm::PointerUnion<clang::Decl *, clang::Stmt *> node) {
    clang::SourceLocation start, end;
    if (clang::Decl *decl = node.dyn_cast<clang::Decl *>()) {
        start = decl->getSourceRange().getBegin();
        end = decl->getSourceRange().getEnd();
    } else if (clang::Stmt *stmt = node.dyn_cast<clang::Stmt *>()) {
        start = stmt->getSourceRange().getBegin();
        end = stmt->getSourceRange().getEnd();
    }
    if (start.isMacroID())
        start = context->getSourceManager().getFileLoc(start);
    if (end.isMacroID())
        end = context->getSourceManager().getFileLoc(end);
